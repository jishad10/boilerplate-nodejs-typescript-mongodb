...
export interface ConflictAfterPaymentParams {
  name: string;
  email: string;
  phone: string;
  eventId: string;
...
export interface PaymentSuccessAdminParams {
  name: string;
  email: string;
  phone: string;
  eventId: string;
...
const verificationCodeTemplate = (code: string): string => `
...
export const getConflictAfterPaymentTemplate = ({
  ...
  paymentIntentId,
  refundAmount,
}: ConflictAfterPaymentParams): string => {
...
export const getPaymentSuccessForAdminTemplate = ({
  ...
  eventId,
  slots,
}: PaymentSuccessAdminParams): string => {
  const slotDetails = slots
    .map(
      (slot, index) =>
        `<li><strong>Slot ${index + 1}:</strong> ${slot.date} from ${slot.startTime} to ${slot.endTime}</li>`
